...
@Component({
  ...
  template: `
    <app-page-header
      title="MLOps Alerts"
      subtitle="Active alerts from Vertex AI pipeline jobs"
      [showRefresh]="true"
    ...
    @if (alertsQuery.isPending()) {
      ...
      <div class="empty-state card card-body">
        ...
        <p>{{ (alertsQuery.data()?.total ?? 0) === 0 ? 'All MLOps jobs are healthy.' : 'No alerts match the current filter.' }}</p>
      </div>
    } @else {
...
export class MlopsAlertsComponent {
  ...
  readonly alertsQuery = injectQuery(() => ({
    queryKey: ['mlops', 'alerts', this.searchTerm, this.severityFilter, this.statusFilter, this.page(), this.pageSize()],
    queryFn: () => this.mlopsApi.getAlerts({
  ...
  acknowledge(id: string): void {
    ...
    this.mlopsApi.acknowledgeAlert(id).then(() => {
